...
export const zhCN: Catalog = {
  ...
  'how.mesh.body':
    '语音、视频和屏幕通过原生 WebRTC 在浏览器之间直接流动，端到端加密。我们的服务器把浏览器互相引荐，并保存房间里有谁的名单 —— 没有可以偷听的媒体服务器，也没有为它付的账单。还有一件事，而且只在有人开口时才发生：把一个页面粘进视频工具，服务器会把那个页面打开一次，从它的标记里读出有什么可以播放，回答你，然后什么都不留。那时也没有媒体经过它 —— 视频由每个浏览器自己从它所在的地方获取。对于严格到无法直连的网络，需要 TURN 中继；我们一台都没有，所以那些少见的连接会直接失败，而不是悄悄经过第三方。',
  ...
  'chat.placeholder': '消息… 支持 Markdown',
  'chat.send': '发送消息',
  'chat.toolbar': '消息格式',
  ...
  'community.promise.vendor.body':
    '没有媒体服务商，也没有第三方 SDK。整套协议都在仓库里，全部运行在免费的基础设施上。唯一可能的例外是可选的 TURN —— 一个为封锁直连的网络准备的中继。它转发的是自己无法解密的加密流量，改一行就能换成自建的。本服务目前没有配置任何 TURN。把另一面也说清楚：当视频工具只能把别人的页面原样放上来时，那个页面会在这里每个人的浏览器里加载，并运行它自己的脚本，对面的站点也就像任何站点那样看得到每个人的连接。设置里的一个开关就能让这些在你这边完全不加载，房间会在没有你的情况下继续看。',
