...
export const jaJP: Catalog = {
  ...
  'how.mesh.body':
    '音声・映像・画面はネイティブの WebRTC でブラウザからブラウザへ、エンドツーエンドで暗号化されて流れます。サーバーがするのはブラウザ同士を引き合わせることと、ルームにいる人の一覧を持つことです — 盗み聞きするメディアサーバーは存在せず、その請求書もありません。もう一つ、しかも誰かが頼んだときだけ：動画ツールにページを貼ると、サーバーはそのページを一度だけ開き、マークアップから再生できるものを読み取り、答えを返し、何も残しません。そのときもメディアはサーバーを通りません — 動画は各ブラウザが元の場所から取得します。直接つなぐには厳しすぎるネットワークには TURN リレーが要りますが、私たちは 1 台も動かしていないので、その稀な接続は第三者を黙って経由せずに失敗します。',
  ...
  'chat.placeholder': 'メッセージ… Markdown が使えます',
  'chat.send': 'メッセージを送信',
  ...
  'community.promise.vendor.body':
    'メディア事業者もサードパーティ SDK もありません。プロトコルはすべてリポジトリにあり、全体が無料のインフラで動いています。唯一あり得る例外は任意の TURN です。直接接続を遮断するネットワーク向けの中継で、復号できない暗号化トラフィックをそのまま転送するだけ。自分で立てる場合も変更は 1 行です。本サービスでは現在どれも設定していません。正直に、裏側も書きます：動画ツールが他人のページをそのまま映すことしかできない場合、そのページはここにいる全員のブラウザで読み込まれ、そのページ自身のスクリプトが動きます。向こうのサイトからは、ふつうのサイトと同じように各自の接続が見えます。設定のスイッチひとつで、あなたのところには何も読み込まれなくなり、ルームはあなた抜きで見続けます。',
