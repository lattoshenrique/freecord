 ...
 *   question is a fingerprint — so the doorstep offers a real
 *   `freecord://…` link (`components/OpenInApp.tsx`). It must stay a link that
 *   somebody clicks: browsers deliberately refuse to launch an external
 *   protocol from a mount effect with no user activation.
 *
